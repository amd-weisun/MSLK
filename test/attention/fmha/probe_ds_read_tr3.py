"""Probe ds_read_tr16_b64 with corrected address formula.

Testing formula: d_col = tr_k_group*4 + (lane%4), m_row = m_base + tr_col_sub

Based on analysis of probe2 results showing:
- Lane at (tr_k, tr_col, tr_ch): result d-col should be lane_mod_32
- Result e-th element m-row should be m_base + e (for lo call)

Hypothesis: the 4x4 transpose transposes the (tr_k_group, tr_col_sub) block,
so m_row in address encodes tr_col_sub and d_col encodes tr_k_group.
"""
import sys
...
def build():
    allocator = SmemAllocator(None, arch="gfx950", global_sym_name="probe_tr3_smem")
    ...
    def kern(C: fx.Tensor):
        ...
        # HYPOTHESIS: d_col = tr_col_half*16 + tr_k_group*4 + (lane%4)
        #             m_row = m_base + tr_col_sub (for lo call)
        #             m_row_hi = m_row_lo + 4
        d_col = tr_col_half * 16 + tr_k_group * 4 + fx.Index(lane % 4)
        m_row_a = fx.Index(M_BASE) + lane_div_32 * 8 + tr_col_sub
...
if __name__ == "__main__":
    ...
    Cv = C.view(64, 8)
    print(f"Probe3: d_col=tr_col_half*16 + tr_k_group*4 + lane%4, m_row=M_BASE + lane_div_32*8 + tr_col_sub")
    print("Expected: e-th result = dO[M_BASE + lane_div_32*8 + e, lane_mod_32]")
    ok = 0
    fail = 0
    for lane in range(64):
        ...
        for e, (m, d) in enumerate(decoded):
            exp_m = M_BASE + lane_div * 8 + e
            exp_d = lane_mod
            if m != exp_m or d != exp_d:
                label = f"FAIL e{e}: got ({m},{d}) want ({exp_m},{exp_d})"
                fail += 1
                break
        else:
            ok += 1
        dec_str = " ".join(f"{m},{d}" for m, d in decoded)
        if label != "OK" or lane < 2 or lane in [4, 16, 32, 48]:
            print(f"  {lane:2d}: {dec_str} | {label}")
    print(f"\nSummary: {ok}/64 lanes OK, {fail} lanes FAIL")
    if fail == 0:
        print("PERFECT - address formula is correct!")
