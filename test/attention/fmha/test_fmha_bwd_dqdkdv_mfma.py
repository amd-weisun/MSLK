"""WP-A3 Phase B.5 — test: FULLY FUSED dQ+dV+dK MFMA kernel vs ref_fmha_bwd.

dQ is accumulated via atomic-add into an f32 scratch (must be zeroed before launch).
...
def run_case(B, M, N, H, D, dtype, device="cuda"):
    ...
    tmpV = torch.zeros_like(dV_out); tmpK = torch.zeros_like(dK_out); tmpQ = torch.zeros_like(dQ_out)
    args_compile = (Q_2d, K_2d, V_2d, dO_2d, tmpV, tmpK, tmpQ, LSE_2d, D_vec,
                    B, M, N, H, n_M_tiles, torch.cuda.current_stream())
    compiled = flyc.compile(launch_fn, *args_compile)

    args = (Q_2d, K_2d, V_2d, dO_2d, dV_out, dK_out, dQ_out, LSE_2d, D_vec,
            B, M, N, H, n_M_tiles, torch.cuda.current_stream())
    compiled(*args)
    torch.cuda.synchronize()

    dV_k = dV_out.view(B, N, H, D)
    dK_k = dK_out.view(B, N, H, D)
    dQ_k = dQ_out.view(B, M, H, D)
    err_v = (dV_k.float() - dV_ref.float()).abs().max().item()
    err_k = (dK_k.float() - dK_ref.float()).abs().max().item()
    err_q = (dQ_k.float() - dQ_ref.float()).abs().max().item()
    ...
    ok_q = torch.allclose(dQ_k.float(), dQ_ref.float(), rtol=0.1, atol=0.1)
    ok = ok_v and ok_k and ok_q
    print(f"{'PASS' if ok else 'FAIL'}  dV={err_v:.4f} dK={err_k:.4f} dQ={err_q:.4f}")
    return ok
